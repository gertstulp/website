---
title: "Kinderloosheid in Nederland"
description: |
...
date: '2023-09-13'
toc: true
image: ../../images/childlessness.png
image-alt: I still have to find a figure for this. 
language: 
    section-title-footnotes: References
---

Voor een podcast werd ik geïnterviewd door [Eveline van Rijswijk](http://evelinevanrijswijk.nl/) over kinderloosheid. Dat was een erg plezier gesprek. Hieronder wat onderwerpen waarover we spraken. 
...
theme_gert <- function () { 
    theme_minimal(base_size = 12, base_family="Avenir") %+replace% 
        theme(
            ...
            plot.background = element_rect(colour = "lightgrey", fill = NA, linewidth = 1)
        )
}
```
...
```{r echo = FALSE}
man_vouw <- tibble(
  geslacht = c("Men", "Women", 
               "Men", "Women", 
               "Men", "Women", 
               "Men", "Women"),
  cohort = c("1945-1949", "1945-1949",
             "1950-1954", "1950-1954",
             "1955-1959", "1955-1959",
             "1960-1964", "1960-1964"),
  kindloos = c(17, 13, 
...
plot <- ggplot(man_vouw, aes(x = cohort, 
                y = kindloos, colour = geslacht)) +
  geom_point(size = 3) +
  ...
  labs(x = NULL, y = "% childlessness", colour = NULL,
       caption = "SOURCE: CBS: Mannen vaker kinderloos, 2010")  +
  guides(fill = "none") 
plot
# to create image for preview
# plot + theme_void() + theme(legend.position = c(0.1, 0.8)) 
# ggsave("childlessness.png", width = 991, height = 589, unit = "px") 
```
...
tfr_col_nms <- read_xlsx("../../datasets_not_online/Human Fertility Database/HFD 2023-08-30/TFR_download_2023_09_13.xlsx", sheet = "Total fertility rates", skip = 1) %>%
  names()
tfr_col_nms[1] <- "PERIOD"
tfr <- read_xlsx("../../datasets_not_online/Human Fertility Database/HFD 2023-08-30/TFR_download_2023_09_13.xlsx", sheet = "Total fertility rates", skip = 2) 
colnames(tfr) <- tfr_col_nms

tfr <- tfr %>% gather(-PERIOD, key = "Country", value = "Fertility") %>% 
  filter(Fertility > 0) 

afb_col_nms <- read_xlsx("../../datasets_not_online/Human Fertility Database/HFD 2023-08-30/MAB1_download_2023_09_13.xlsx", sheet = "Mean age at first birth", skip = 1) %>%
  names()
afb_col_nms[1] <- "PERIOD"
afb <- read_xlsx("../../datasets_not_online/Human Fertility Database/HFD 2023-08-30/MAB1_download_2023_09_13.xlsx", sheet = "Mean age at first birth", skip = 2)
colnames(afb) <- afb_col_nms

afb <- afb %>% 
  gather(-PERIOD, key = "Country", value = "AFB") %>% 
  filter(AFB > 0) 
  
chi_col_nms <- read_xlsx("../../datasets_not_online/Human Fertility Database/HFD 2023-08-30/PARITY_downloaded_2023-09-30_HFD.xlsx", sheet = "Cohort childlessness", skip = 1) %>%
  names()
chi_col_nms[1] <- "PERIOD"
chi <- read_xlsx("../../datasets_not_online/Human Fertility Database/HFD 2023-08-30/PARITY_downloaded_2023-09-30_HFD.xlsx", sheet = "Cohort childlessness", skip = 2)
colnames(chi) <- chi_col_nms

chi <- chi %>% 
  gather(-PERIOD, key = "Country", value = "chi") %>% 
  filter(chi > 0) 


ggplot(chi, aes(x = PERIOD, y = chi, group = Country)) +
  geom_line(colour = "grey") +
  geom_line(data = filter(chi, Country == "Netherlands"),
            colour = "Orange", linewidth = 2) +
  scale_y_continuous(limits = c(0, 30), breaks = seq(0, 35, 10)) +
  scale_x_continuous(breaks = seq(1910, 2020, 10)) +
  labs(x = NULL, y = "% childlessness") +
  ggplot(tfr, aes(x = PERIOD, y = Fertility, group = Country)) +
  geom_line(colour = "grey") +
  geom_line(data = filter(tfr, Country == "Netherlands"),
            colour = "Orange", linewidth = 2) +
  scale_y_continuous(limits = c(0, 5)) +
  scale_x_continuous(breaks = seq(1960, 2025, 10)) +
  labs(x = NULL, y = "fertility") +
  ggplot(afb, aes(x = PERIOD, y = AFB, group = Country)) +
  geom_line(colour = "grey") +
  geom_line(data = filter(afb, Country == "Netherlands"),
            colour = "Orange", linewidth = 2) +
  scale_y_continuous(limits = c(20, 33), breaks = seq(20, 32, 4)) +
  scale_x_continuous(breaks = seq(1960, 2020, 10))  +
  labs(x = NULL, y = "age at first birth", 
...
```{r echo = FALSE}
ggplot(filter(chi, PERIOD == 1975), 
       aes(x = fct_reorder(Country, chi), y = chi)) +
  geom_bar(stat = "identity", fill = "grey") +
  geom_bar(data = filter(chi, Country == "Netherlands" & PERIOD == 1975),
           stat = "identity", fill = "Orange", linewidth = 2) +
  #scale_y_continuous(limits = c(0, 30), breaks = seq(0, 35, 10)) +
  # scale_x_continuous(breaks = seq(1910, 2020, 10)) +
  theme_minimal() +
...
```{r echo = FALSE}
opl <- tibble(
  Geslacht = c("Men", "Men", "Men",
               "Women", "Women", "Women"),
  Edu = c("Low", "Medium", "High",
          "Low", "Medium", "High"),
  percentage = c(100-67.2, 100-71.6, 100-77.7, 
                 100-85.5, 100-82.1, 100-79.2)
)

ggplot(opl, aes(x = forcats::fct_inorder(Edu), 
                y = percentage, fill = Geslacht)) +
  geom_bar(stat = "identity") +
  facet_grid(~factor(Geslacht, levels = c("Women", "Men"), ordered = TRUE)) +
  ...
  labs(x = NULL, y = "% childless", 
       caption = "SOURCE: CBS: Laagopgeleide mannen vaker kinderloos, 2017")  +
  guides(fill = "none") 
...
```{r}
vrijw <- tibble(
  Cohort = c("1935-1944", "1935-1944", "1935-1944", 
             "1945-1954", "1945-1954", "1945-1954", 
             "1955-1964", "1955-1964", "1955-1964"),
  category = c("Childfree", "Wished to have children", "Other",
               "Childfree", "Wished to have children", "Other",
               "Childfree", "Wished to have children", "Other"),
  cohort_chil = c(12, 12, 12, 
                  12, 12, 12,
                  18, 18, 18),
  percentage = c(29, 79-29, 100-79,
                 45, 87-45, 100-87,
                 47, 94-47, 100-94),
  rel = cohort_chil * (percentage/100)
)
...
ggplot(vrijw, 
       aes(x = factor(Cohort), 
           ...
           fill = factor(category, 
                         levels = rev(c("Childfree", "Wished to have children", "Other")),
                         ordered = TRUE))) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(
    data = filter(vrijw, category == "Childfree"),
    aes(label = paste0(percentage, "%")),
    size = 5, colour = "white", vjust = 1
  ) +
  scale_fill_brewer(
    palette = "Set2",
  ...
  labs(x = NULL, y = "% childless", fill = NULL, source = "CBS, Kinderen krijgen, 2023") 
```
...
```{r echo = FALSE}
ryb <- read_xlsx("../../datasets_not_online/datasets_blog/Rybinska.xlsx")

ggplot(ryb, aes(x = Jaar, y = Proportie, colour = Leeftijd)) +
  geom_point(size = 3) +
  geom_line(aes(group = Leeftijd)) +
  facet_grid(~factor(Geslacht, levels = c("Women", "Men"), ordered = TRUE)) +
  scale_y_continuous(limits = c(0, 25)) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_colour_viridis_d(limits = (c("40-44", "35-39", "30-34", "25-29", "18-24"))) +
  theme_gert() +
  #theme_minimal() +
  theme(
    panel.grid.major.x = element_blank(),
  ...
  labs(x = NULL, y = "% intend to remain childless", 
       caption = "SOURCE: Rybinska, 2020") 
...
```{r}
cha <- read_xlsx("../../datasets_not_online/datasets_blog/Chancey.xlsx")

ggplot(cha, aes(x = Jaar, y = Proportie, colour = Leeftijd)) +
  geom_point(size = 3) +
  geom_line(aes(group = Leeftijd)) +
  facet_grid(~Category) +
  scale_y_continuous(limits = c(0, 30)) +
...
```{r}
heat <- read_xlsx("../../datasets_not_online/datasets_blog/Heaton.xlsx") %>% 
  mutate(percentage = paste0(round(100 *N/sum(N)), "%"))

ggplot(heat, aes(x = reorder(Label, N), y = N)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  geom_text(aes(label = percentage), colour = "white", hjust = 1) +
  theme_gert() +
  theme(
    panel.grid.major.y = element_blank(),
    axis.text.y = element_text(size = 14)
  ) +
  labs(x = NULL, y = "N", caption = "SOURCE: Heaton et al., XXX")  +
  guides(fill = "none") +
  coord_flip()
```
...
```{r}
verw <- tibble(
  reden = c("Vrijwillig kinderloos",
            "Onvrijwillig kinderloos, wel vruchtbaar",
            "Onvruchtbaar na omstandigheden",
            "Onverantwoord door ziekte/handicap",
            "Altijd onvruchtbaar geweest",
            "Andere redenen"),
  percentage = c(55, 11, 5, 7, 9, 13)
)

ggplot(verw, aes(x = reorder(reden, percentage), y = percentage)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  geom_text(aes(label = paste0(percentage, "%")), colour = "white", hjust = 1) +
  theme_gert() +
  theme(
    panel.grid.major.y = element_blank(),
    axis.text.y = element_text(size = 14)
  ) +
  labs(x = NULL, y = "%", caption = "SOURCE: de Graaf & Loozen, 2005")  +
  guides(fill = "none") +
  coord_flip()
```
...
Redenen om geen kinderen te hebben vanuit CBS: Arie de Graaf
Bron: Onderzoek Gezinsvorming, 2003.
```{r}
reden <- tibble(
  reden = c("Hard to arrange childcare",
            "Children cost too much money",
            "Health issues within couple",
            "No desire/not suitable",
            "Combining work/family difficult",
            "Partner did not want children",
            "Raising children is too much effort/energy",
            "Children obstruct freedom"),
  percentage = c(5, 7, 18, 22, 26, 29, 33, 54)
)

ggplot(reden, aes(x = fct_inorder(reden), y = percentage)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  ...
  labs(x = NULL, y = "%", caption = "CBS: Graaf, 2003")  +
  guides(fill = "none") +
  coord_flip()
```
...
library(tidyverse)
library(readxl)
noort <- read_xlsx("../../datasets_not_online/datasets_blog/Noorthuizen data.xlsx")

ggplot(noort, 
       aes(x = factor(Year), 
           y = Percent, 
           fill = factor(Outcome, 
                         levels = rev(c("Approve", "It depends" ,"Disapprove")),
                         ordered = TRUE))) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(
    data = filter(noort, Outcome == "Approve"),
    aes(label = Percent),
    size = 5, colour = "white", vjust = 1
  ) +
  ...
  labs(x = NULL, y = "%", fill = NULL, source = "Noorthuizen, ") 
```
...
```{r}
bal <- read_xlsx("../../datasets_not_online/datasets_blog/Van Balen and Trimbos.xlsx")

ggplot(bal, aes(x = forcats::fct_rev(forcats::fct_inorder(Motive)), 
                y = Percentage, fill = Sex)) +
  geom_bar(stat = "identity") +
  facet_grid(~factor(Sex, levels = c("Women", "Men"), ordered = TRUE)) +
  ...
  labs(x = NULL, y = "%", source = "CBS: Van Balen & Trimbos")  +
  guides(fill = "none") +
  coord_flip()
```
...
```{r}
prog <- tibble(
  Cohort = seq(1935, 2015, 10),
  percentage = c(11.7, 11.7, 17.1, 18.1, 16.7, 17.2, 18, 18, 18)
)

ggplot(prog, aes(x = Cohort, y = percentage)) +
  geom_point(size = 12, colour = "Orange") +
  geom_line(aes(group = 1), colour = "Orange") +
  geom_text(aes(label = paste0(percentage, "%")), colour = "white", size = 3) +
  scale_x_continuous(breaks = seq(1935, 2015, 10)) +
  ...
  labs(x = NULL, y = "%", caption = "SOURCE: CBS, Kinderen krijgen, 2022") 
```